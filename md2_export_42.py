...
bl_info = {
    ...
    "author": "Sebastian Lieberknecht, Dao Nguyen and Bernd Meyer metaio GmbH (based on Damien Thebault and Erwan Mathieu's Blender2.4x exporter), Ivan Strizhkin (Ligul) - updated for Blender 4.2+",
    "version": (1, 6),
    "blender": (4, 2, 0),
    "api": 46461,
...
class MD2:
	...
	def write(self, filename):
		...
		try:
			...
			# define mesh UV data for modern Blender
			mesh_uv_data = []
			if len(mesh.uv_layers) != 0:
				uv_layer = mesh.uv_layers[0]
				for poly in mesh.polygons:
					if len(poly.vertices) == 3:  # triangles only
						uvs = []
						for loop_index in poly.loop_indices:
							uv = uv_layer.data[loop_index].uv
							uvs.append(uv)
						mesh_uv_data.append(uvs)
					else:
						# For non-triangles, use default UVs
						mesh_uv_data.append([(0, 0), (0, 0), (0, 0)])
			else:
				for poly in mesh.polygons:
					mesh_uv_data.append([(0, 0), (0, 0), (0, 0)])
				
			for uvs in mesh_uv_data:
			...
			# gl commands
			for i, uvs in enumerate(mesh_uv_data):
				bin = struct.pack('<i', 3)
				file.write(bin)
				# Get the corresponding face for vertex indices
	...
	def outFrame(self, file, frameName = 'frame'):
		# Update to modern Blender API
		depsgraph = bpy.context.evaluated_depsgraph_get()
		eval_obj = self.object.evaluated_get(depsgraph)
		...
		for material in mesh.materials:
			...
			for node in material.node_tree.nodes if material.node_tree else []:
				if node.type == 'TEX_IMAGE' and node.image:
					skins.append(node.image.filepath)
	...
	def __init__(self, object):
		...
		if self.ismesh:
			...
			try:
				...
				self.cPolygons = len(mesh.polygons)
...
class Export_MD2(bpy.types.Operator, ExportHelper):
	...
	def execute(self, context):
		...
		# Create ObjectInfo now, when it's safe to call operators
	...
	def invoke(self, context, event):
		...
		# Simple check without creating ObjectInfo to avoid operator calls
		if obj.type == 'MESH':
			mesh = obj.data
			cFinalTriangles = len(mesh.polygons)
...
# List all classes that need to be registered
classes = [
	Export_MD2,
]
